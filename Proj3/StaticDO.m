...
setappdata(0,'instantDoCtrl',instantDoCtrl);
try
...
%     end
    ...
    if (strcmp(data.ControlMode,'Hard'))  
       start(OutputTimer);
       setappdata(0,'OutputTimer',OutputTimer);
    else
        setappdata(0,'OutputTimer',OutputTimer); 
    end
...
if (data.Duration ~= -1 && time > data.Duration)
    time = 0;
    i = 0;
    handles = getappdata(0,'GUIhandle');
    ...
    OutputTimer = getappdata(0,'OutputTimer');
    stop(OutputTimer);
    delete(OutputTimer);
    setappdata(0,'OutputTimer',OutputTimer);
    return;
