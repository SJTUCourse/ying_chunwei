...
setappdata(Handle2,'GUIhandle',handles);
...
if (data.Flag == 3)
    setappdata(Handle2,'time',[]);
    setappdata(Handle2,'i',[]);
    StaticAO();
    errStr = getappdata(Handle2,'errStr');
    if (isempty(errStr))
...
instantAoCtrl = getappdata(Handle2,'instantAoCtrl');
if (isa(instantAoCtrl,'Automation.BDaq.InstantAoCtrl'))
